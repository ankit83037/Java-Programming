...
public class Bazaar {
	...
	public List<Integer> searchBasedOnPolicyType(String policyType){
	    List<Integer> policyList = new ArrayList<>();
		    ...
		    for(Map.Entry e: this.policyMap.entrySet()){
		        if(e.getValue().toString().contains(policyType)){
		            policyList.add((int)e.getKey());
		        }
		      //  else if(e.getValue().toString().contains("Two Wheeler")){
		      //       policyList.add((int)e.getKey());
		      //  }
		    }
